...
def compute(P, v, is_reflex=True):
	"""
...
#	print P
	...
	if is_reflex:
		if theta_l < 0 and theta_r < 0:
			...
			print("ERROR: CONE OF BISECTION<: IF DID NOT CAPTURE")
	else:
		if theta_l <= 0 and theta_r < 0:
			angle = abs(theta_l-theta_r)
			orientation = theta_r+angle/2
		elif theta_l > 0 and theta_r <= 0:
			angle = theta_l-theta_r
			orientation = theta_r+angle/2
		elif theta_l > 0  and theta_r > 0:
			angle = theta_l-theta_r
			orientation = theta_r+angle/2
		elif theta_l <= 0 and theta_r > 0:
			angle = 2*pi-(theta_r-theta_l)
			orientation = theta_r+angle/2
...
if __name__ == '__main__':
	...
	import adjacency_edges as adj_e
